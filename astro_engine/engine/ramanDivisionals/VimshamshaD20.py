...
def get_nakshatra(sidereal_lon):
    """
    Calculate the nakshatra (star) from sidereal longitude.
...
def raman_vimsamsa(data):
    """
    Calculate the D20 (Vimsamsa) chart with nakshatras.
    
    Args:
        data (dict): Input data including birth_date, birth_time, latitude, longitude, timezone_offset, user_name
    
    Returns:
        dict: D20 chart details including planetary positions, ascendant, and house signs
    """
    ...
    # Step 8: Construct and return JSON response
    response = {
