...
def get_nakshatra(longitude):
    ...
    # logger.debug(f"Longitude {longitude:.6f}: Nakshatra {nakshatra}")
    return nakshatra

def calculate_d3_chart(jd, lat, lon):
    """Calculate D3 chart data with retrograde status and nakshatras."""
    swe.set_sid_mode(swe.SIDM_RAMAN)
...
def raman_dreshkana(data):
    """Main calculation function for D3 chart with nakshatras."""
