...
def get_d4_position(d1_lon):
    ...
    # Correct offsets for D4 sign calculation (1st, 4th, 7th, 10th signs)
    offsets = [0, 3, 6, 9]
    d4_sign_index = (d1_sign_index + offsets[segment]) % 12

    # Calculate D4 degree within the sign
...
def raman_chaturthamsha(data):
    """
    Calculate the Chaturthamsha (D4) chart with nakshatras.
    
    Args:
        data (dict): Input data with birth_date, birth_time, latitude, longitude, timezone_offset
    
    Returns:
        dict: Chaturthamsha chart data with planetary positions and ascendant
    """
    # Parse inputs
