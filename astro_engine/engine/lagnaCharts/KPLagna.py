...
def convert_to_julian_day(date_str, time_str, tz_offset):
...
def fetch_planet_positions(jd):
    ...
    for planet, pid in planet_ids.items():
        ...
        if not isinstance(result, tuple) or len(result) < 1:
            logging.error(f"Failed to compute position for {planet}: {result}")
            raise ValueError(f"Invalid result from swe.calc_ut for {planet}")
        positions[planet] = result[0][0] % 360
        speed = result[0][3]  # Speed in longitude
        positions[planet + '_retro'] = speed < 0
    
    # Calculate Ketu's position
    positions['Ketu'] = (positions['Rahu'] + 180) % 360
    positions['Ketu_retro'] = positions['Rahu_retro']
    return positions

def fetch_house_cusps(jd, latitude, longitude):
    ...
    if not isinstance(house_pos, tuple) or len(house_pos) < 2:
        logging.error(f"Failed to compute house cusps: {house_pos}")
        raise ValueError("Invalid result from swe.houses_ex")
    cusps = [cusp % 360 for cusp in house_pos[0]]
    return cusps

def identify_sign(longitude):
    """Determine zodiac sign from longitude."""
    sign_index = floor(longitude / 30) % 12
    return SIGNS[sign_index]

def identify_nakshatra(longitude):
...
def identify_sub_lord(longitude):
    """Determine sub-lord from longitude within nakshatra."""
...
def map_planets_to_houses(planet_positions, house_cusps):
    """Map planets to houses based on their positions and cusp ranges."""
    house_assignments = {}
...
def determine_significators(planet_positions, house_cusps):
    """Determine significators for each house using KP rules."""
    ...
    for house in range(1, 13):
        ...
        nak = identify_nakshatra(planet_positions[house_lord])
        nak_lord = nakshatra_lords[nak]
        if nak_lord not in significators[house_key]:
            significators[house_key].append(nak_lord)
